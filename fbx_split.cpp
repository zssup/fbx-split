 ...
 * Save FbxScene
 */
 // 辅助函数：将字符串中的所有 old_char 替换为 new_char
std::string ReplaceChar(std::string s, char old_char, char new_char) {
...
void CheckFolder(const char* dir) {
    ...
    if (attrib != INVALID_FILE_ATTRIBUTES && (attrib & FILE_ATTRIBUTE_DIRECTORY)) {
        ...
        if (CreateDirectoryA(dir, NULL)) {
            std::cout << "create folder: " << dir << std::endl;
        }
        else {
            DWORD error = GetLastError();
            if (error == ERROR_ALREADY_EXISTS) {
                std::cout << "folder already exists: " << dir << std::endl;
            }
            else {
...
void FbxSplit(const char* path, const char* outputFolder) {
    ...
    for (int i = 0; i < scenes.size(); i++) {
        CommonSaveScene(lSdkManager, scenes[i], CombinePath(outputFolder, ReplaceChar(names[i], '/', '_').c_str()),  0);
